...
class OptionsBot:
    ...
    async def check_messages(self):
        ...
        if message is not None and message['type'] == 'message':
            ...
            if action == constants.BUY:
                if symbol == constants.AMAZON:
                        ...
                        if right == constants.CALL:
                            ...
                            if condition == "breakout":
                                ...
                                self.sma_amazon_call_options_contract = self.create_options_contract(
                                    ...
                                    right)
                                options_order = MarketOrder(action, contracts)
                                placed_order = self.ib.placeOrder(self.sma_amazon_call_options_contract, options_order)
                                self.insert_option_contract(condition, self.sma_amazon_call_options_contract)
                                print(self.sma_amazon_call_options_contract)
                                print("Options order to place:", options_order)
                                print("Final Placed order:", placed_order)
                            ...
                            if condition == "breakout":
                                self.breakout_amazon_put_options_contract = self.create_options_contract(
                                    ...
                                    right)
                                options_order = MarketOrder(action, contracts)
                                placed_order = self.ib.placeOrder(self.breakout_amazon_put_options_contract,
                                                                  options_order)
                                self.insert_option_contract(condition, self.breakout_amazon_put_options_contract)
                                print("Options order to place:", options_order)
                                print("Final Placed order:", placed_order)
                            elif condition == "sma":
                                self.sma_amazon_put_options_contract = self.create_options_contract(symbol,
                                                                                                    options_chain.expirations[
                                                                                                        0],
                                                                                                    put_strike, right)
                                options_order = MarketOrder(action, contracts)
                                placed_order = self.ib.placeOrder(self.sma_amazon_put_options_contract, options_order)
                                self.insert_option_contract(condition, self.sma_amazon_put_options_contract)
                                print("Options order to place:", options_order)
                                print("Final Placed order:", placed_order)
                    ...
                    for options_chain in self.nvidia_option_chains:
                        ...
                        if right == constants.CALL:
                            call_strike = call_strikes[6]
                            if condition == "breakout":
                                self.breakout_nvidia_call_options_contract = self.create_options_contract(symbol,
                                                                                                          options_chain.expirations[
                                                                                                              0],
                                                                                                          call_strike,
                                                                                                          right)
                                options_order = MarketOrder(action, contracts)
                                placed_order = self.ib.placeOrder(self.breakout_nvidia_call_options_contract,
                                                                  options_order)
                                self.insert_option_contract(condition, self.breakout_nvidia_call_options_contract)
                                print("Options order to place:", options_order)
                                print("Final Placed order:", placed_order)
                            elif condition == "sma":
                                self.sma_nvidia_call_options_contract = self.create_options_contract(symbol,
                                                                                                     options_chain.expirations[
                                                                                                         0],
                                                                                                     call_strike, right)
                                options_order = MarketOrder(action, contracts)
                                placed_order = self.ib.placeOrder(self.sma_nvidia_call_options_contract, options_order)
                                self.insert_option_contract(condition, self.sma_nvidia_call_options_contract)
                                print("Options order to place:", options_order)
                                print("Final Placed order:", placed_order)
                            ...
                            if condition == "breakout":
                                self.breakout_nvidia_put_options_contract = self.create_options_contract(symbol,
                                                                                                         options_chain.expirations[
                                                                                                             0],
                                                                                                         put_strike,
                                                                                                         right)
                                options_order = MarketOrder(action, contracts)
                                placed_order = self.ib.placeOrder(self.breakout_nvidia_put_options_contract,
                                                                  options_order)
                                self.insert_option_contract(condition, self.breakout_nvidia_put_options_contract)
                                print("Options order to place:", options_order)
                                print("Final Placed order:", placed_order)
                            elif condition == "sma":
                                self.sma_nvidia_put_options_contract = self.create_options_contract(
                                    ...
                                    right)
                                options_order = MarketOrder(action, contracts)
                                placed_order = self.ib.placeOrder(self.sma_nvidia_put_options_contract, options_order)
                                self.insert_option_contract(condition, self.sma_nvidia_put_options_contract)
                                print("Options order to place:", options_order)
                                print("Final Placed order:", placed_order)
                ...
                selling_contract = None

                if symbol == constants.AMAZON:
                    if condition == "breakout":
                        if right == "CALL":
                            selling_contract = self.breakout_amazon_call_options_contract
                        if right == "PUT":
                            selling_contract = self.breakout_amazon_put_options_contract
                    if condition == "sma":
                        if right == "CALL":
                            selling_contract = self.sma_amazon_call_options_contract
                        if right == "PUT":
                            selling_contract = self.sma_amazon_put_options_contract
                elif symbol == constants.NVIDIA:
                    if condition == "breakout":
                        if right == "CALL":
                            selling_contract = self.breakout_nvidia_call_options_contract
                        if right == "PUT":
                            selling_contract = self.breakout_nvidia_put_options_contract
                    if condition == "sma":
                        if right == "CALL":
                            selling_contract = self.sma_nvidia_call_options_contract
                        if right == "PUT":
                            selling_contract = self.sma_nvidia_put_options_contract

                if selling_contract is None:
                    retrieved_contract = self.check_options_table(condition, symbol)

                    if retrieved_contract:
                        Option
                        print(retrieved_contract)
                        self.ib.placeOrder(retrieved_contract, options_order)
                        print("Sold by retrieving it from Database!")
                    else:
                        print("No Contract to sell with given symbol:", symbol, "and condition:", condition)
                else:
                    print(selling_contract)
                    self.ib.placeOrder(selling_contract, options_order)
                    print("Sold!")
            else:
    ...
    def create_options_contract(self, symbol, expiration, strike, right):
        print("Creating Option Contract...")
    ...
    def update_data(self, result, condition, action):
        cursor = self.conn.cursor()
    ...
    def check_options_table(self, condition, symbol):
        cursor = self.conn.cursor()
        print("Checking for Options contract...")
        sqlite_insert_with_param = constants.SELECT_OPTION
        sqlite_data = (
            condition,
            symbol
        )
        ...
        if found_record:
            print("Found Record:")
            print(found_record[1])
            print(found_record[2])
            print(found_record[3])
            print(found_record[4])
            print(found_record[5])
            print(found_record[6])
            ...
            return Option(
                ...
                found_record[4],
                exchange='SMART',
                tradingClass=found_record[6]
            )
        else:
            print("Didn't find a record for symbol:", symbol, "and condition:", condition)
